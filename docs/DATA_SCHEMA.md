...
Parody (and, someday, real) advertising units for the Sponsors page.
  ...
  "sponsors": [
    {
      "id": "sponsor-durtpoo",
      "name": "Durtpoo",
      "tagline": "It'll remove the soft.",
      "category": "personal care",
      "description": "Ad body copy",
      "model": "SnowMan",
      "testimonial": "Quote from a satisfied customer",
      "testimonialAttribution": "SnowMan, compensated entirely in shampoo",
      "image": "/assets/images/sponsors/durtpoo.png",
      "imageAlt": "Description of the ad creative for accessibility",
      "link": "#",
      "linkText": "Available Nowhere",
      "fineprint": "Durtpoo is not a real product.",
      "badge": "new",
      "featured": true,
      "parody": true
    }
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `id` | string | Yes | Unique identifier with `sponsor-` prefix |
| `name` | string | Yes | Brand name shown as the card heading |
| `tagline` | string | No | Short italic slogan under the name |
| `category` | string | No | Product category (personal care, drinkware, etc.) |
| `description` | string | Yes | Ad body copy |
| `model` | string | No | Band member featured in the creative |
| `testimonial` | string | No | Pull quote rendered in a `<blockquote>` |
| `testimonialAttribution` | string | No | Byline for the testimonial (omit and no byline renders) |
| `image` | string | Yes | Path to the ad creative (PNG; WebP resolved automatically) |
| `imageAlt` | string | Yes | Alt text for accessibility |
| `link` | string | Yes | Destination URL, or `"#"` for none |
| `linkText` | string | No | CTA label (default: `Learn More`, or `Available Nowhere` when inert) |
| `fineprint` | string | No | Small disclaimer under the CTA |
| `badge` | string | No | One of: `new`, `sold`, `wow`, `hot`, or null |
| `featured` | boolean | No | Sorts to the top and gets the brass frame (default: false) |
| `parody` | boolean | No | Marks the ad as fictional; exposed as `data-parody` (default: false) |

**Link behavior:** when `link` is `"#"` the CTA renders as a non-focusable
`<span>` rather than an anchor, so keyboard users are not sent to a control
that does nothing.
...
## Schema: board.json / board-seed.json

Notes on the message board at `/board/`. Two files, same schema:

- **`board-seed.json`** — hand-authored notes. Edit these by hand; the sync
  script never touches them.
- **`board.json`** — **generated.** `board-seed.json` merged with approved
  GitHub issues by `scripts/sync-board.js`. This is the file the site fetches.
...
```json
{
  "posts": [
    {
      "id": "board-house-rules",
      "date": "2026-09-01",
      "name": "The Management",
      "message": "RULES OF THE BOARD:\n1. Write something.\n2. We read it eventually.",
      "style": "flyer",
      "pinned": true,
      "reply": {
        "from": "DeadBeat",
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `id` | string | Yes | Unique identifier. Seed notes use `board-<slug>`; generated notes use `board-<issue number>`. Also seeds the note's appearance |
| `date` | string | Yes | ISO 8601 (`YYYY-MM-DD`). Sorts the board, newest first |
| `name` | string | Yes | Who wrote it. Max 40 characters. Empty submissions become `Anonymous` |
| `message` | string | Yes | The note. Max 600 characters. `\n` renders as a line break |
| `style` | string | No | Forces a paper style. Omit to pick one deterministically from `id` |
| `pinned` | boolean | No | `true` sorts the note above all unpinned notes. Default `false` |
| `reply` | object | No | The band's response, rendered in red marker under the note |

### Reply Schema

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `from` | string | No | Signer. `DeadBeat`, `SnowMan`, or `The Management` |
| `date` | string | No | ISO 8601 date of the reply |
| `text` | string | Yes | The reply itself |

### Style Values

`index`, `lined`, `sticky`, `napkin`, `receipt`, `flyer`, `card`

Anything else falls back to a seeded pick. Pin color, fastener, and rotation
are always seeded from `id` — the same note looks identical on every load.

> **Note:** `message`, `name`, and `reply.text` can contain visitor-submitted
> text. Every field is escaped with `DurtNursUtils.escapeHTML()` at render
> time, sanitized again in the Worker, and once more in the sync script. Any
> new consumer of this data must escape it too.

See `docs/MESSAGE_BOARD.md` for the moderation flow.
